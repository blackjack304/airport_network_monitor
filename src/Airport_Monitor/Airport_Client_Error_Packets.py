...
#Author: Niket Shah
"""
Munin plugin to monitor error packets sent/recd by wireless client's
connected to an Apple Airport Express/Extreme or a Time Capsule.
Author: Niket Shah

To use this plugin:
place this python script (along with 'Airport_Monitor.py') to /etc/munin/plugins folder.

this will create a virtual host in munin for the airport base station and 
produce graphs for:
* Error packets sent/recd by wireless client's
"""
...
def main():
    ...
    if len(sys.argv) > 1 and sys.argv[1] == 'config':
        ...
        print "graph_title Wireless Client's error packets"
        print "graph_vlabel bits per second"
        print """graph_info This graph shows error packets sent/received by each\
 Wireless Client Interface (Only if supported by client)"""

        clients = R0.Get_Data()
        for client in clients:
            if int(clients[client]['rxerr']) > -1 and int(clients[client]['txerr']) > -1:
        ...
        for client in clients:
            if int(clients[client]['rxerr']) > -1 and int(clients[client]['txerr']) > -1:
                print "MAC_%s_send.value %s" % (client, int(clients[client]['txerr']))
                print "MAC_%s_recv.value %s" % (client, int(clients[client]['rxerr']))
